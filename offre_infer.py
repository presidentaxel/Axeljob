...
# Noms trop courts / faux positifs fréquents en tête d'annonce
...
# Phrases typiques d'accroche / baseline (pas une raison sociale)
_SLOGAN_MARKERS = (
    "taille humaine",
    "grand groupe",
    "force d'un",
    ...
    "banque publique",  # souvent dans une phrase descriptive, pas le nom court
)

# Mots-outils : trop nombreux → probablement une phrase, pas « Société X »
_STOPISH = frozenset(
    {
        "une",
        "des",
...
def _clean_company(raw: str) -> str:
    ...
    s = re.sub(r"\s+", " ", s)
    s = s.strip(" \t\"'«»•-–—:|")
    # Couper si phrase trop longue (souvent description collée au label)
    if len(s) > 90:
        s = s[:90].rsplit(" ", 1)[0]
    return s.strip()
...
def _looks_like_slogan_or_phrase(name: str) -> bool:
    """Filtre les baselines RH et morceaux de phrase capturés par erreur."""
...
def infer_entreprise_from_annonce(text: str) -> tuple[str, float]:
    """
    Devine l'entreprise à partir du texte de l'offre.
    Confiance indicative : 1.0 = explicite (label), ~0.85 motif « X recrute », etc.
    """
    if not text or not str(text).strip():
        return "", 0.0

    t = text.strip()
    # --- Labels explicites (forte confiance) ---
    for label, conf in (
        (r"(?:^|\n)\s*Entreprise\s*:\s*([^\n]+)", 0.92),
        (r"(?:^|\n)\s*Employeur\s*:\s*([^\n]+)", 0.92),
        (r"(?:^|\n)\s*Société\s*:\s*([^\n]+)", 0.9),
    ...
    # --- « Travailler chez X », « Postuler chez X » (souvent le vrai employeur) ---
    for pat, conf in (
    ...
    # --- « chez X » (générique ; après les formules explicites ci-dessus) ---
    m = re.search(
    ...
    # --- « La société X » en début de phrase (PAS « grand groupe » + reste de baseline) ---
    m = re.search(
    ...
    # --- « le groupe X » / « notre groupe X » avec nom court (évite GRAND GROUPE + phrase) ---
    m = re.search(
    ...
    # --- « X recrute » (début de ligne ou après saut) ---
    m = re.search(
    ...
    # --- Ligne 1 : « Titre — Entreprise » ou « Titre | Entreprise » (confiance modérée) ---
    first = t.split("\n", 1)[0].strip()
    for sep in (" — ", " – ", " - ", " | ", " / "):
        if sep in first:
            parts = [p.strip() for p in first.split(sep) if p.strip()]
            if len(parts) >= 2:
                # Souvent titre puis entreprise (prendre le dernier segment court)
                candidate = parts[-1]
