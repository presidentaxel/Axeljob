...
5. Classes CSS obligatoires (pour que les réglages typo/couleurs de l'utilisateur s'appliquent) : conteneur principal avec class="cv". En-tête : .cv-header ou .main-header ; nom avec .header-nom ou .sidebar-nom ou dans .main-header h1 ; titre pro avec .header-titre, .sidebar-titre ou .main-header p ; titres de section avec .section-title, .main-section-title, .sidebar-section-title ou h2 dans .cv ; corps avec .cv-main, .resume-text ou .right-column, .profil p ; expériences avec .experience-item, .exp-poste, .bullet ou .timeline-content h3, .timeline-content .company, .timeline-content .date, .timeline-content .bullets li ; sidebar avec .cv-sidebar, .sidebar-item ou .left-column, .contact ul li. Dans le CSS, définis dans :root les variables (--cv-fs-name, --cv-fs-title, --cv-fs-section, --cv-fs-body, --cv-fs-bullet, --cv-fs-sidebar-item, --cv-color-body, --cv-color-section-title, --cv-header-color, --cv-sidebar-color, --cv-accent-color, --cv-font-heading, --cv-photo-size pour la taille de la photo en px) et utilise-les partout (ex. font-size: var(--cv-fs-name, 15pt); color: var(--cv-color-body, #1a1a1a)) pour body, h1, h2, texte et fonds.
{JINJA_VARS_REF}
