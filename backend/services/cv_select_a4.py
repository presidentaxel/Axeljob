...
def _build_select_prompt(cv: dict, offre: dict | None) -> str:
    ...
    if isinstance(logiciels, list):
        logiciels = [str(sw).strip() for sw in logiciels if sw]
    else:
        logiciels = []
...
<langues>
{json.dumps([(lg.get("langue") or "") + " - " + (lg.get("niveau") or "") for lg in langues[:10] if isinstance(lg, dict)], ensure_ascii=False)}
</langues>
</cv>
{offre_block}
...
def _should_skip_selection(cv: dict) -> bool:
    ...
    n_exp = len(
        [
            ...
            if e.get("poste") or e.get("entreprise") or (e.get("bullet_points") or [])
        ]
    )
    n_form = len(
        [
            f
            for f in (cv.get("formations") or [])
            if f.get("diplome") or f.get("etablissement") or f.get("date")
        ]
    )
    ...
    return n_exp <= 6 and n_form <= 5 and n_proj <= 5 and (n_tech + n_log) <= 15
...
def select_cv_content_for_a4(
    ...
    *,
    ...
    sel = _extract_json((r.text or "").strip())
